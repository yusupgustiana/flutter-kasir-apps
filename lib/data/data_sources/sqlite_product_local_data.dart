...
class ProductLocalData {
  ...
  Future _createDB(Database db, int version) async {
    await db.execute(
        'CREATE TABLE $tableProducts(Id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, category String, price INTEGER, image TEXT, stock INTEGER, best_seller INTEGER)');
  }

  Future<Database> get database async {
    if (_database != null) return _database!;

    _database = await _initDB('product2.db');
    return _database!;
  }
